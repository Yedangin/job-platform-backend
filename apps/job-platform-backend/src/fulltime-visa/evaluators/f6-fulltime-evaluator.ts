/**
 * F-6 결혼이민비자 정규직 평가기
 * F-6 Marriage Immigration Visa Fulltime Evaluator
 *
 * F-6 핵심: 내국인과 동일한 취업 권리. 모든 직종, 모든 조건에서 채용 가능.
 * F-6 core: Same employment rights as Korean nationals. Hireable for all occupations and conditions.
 *
 * [특징 / Characteristics]
 ...
 *
 * [법적 근거 / Legal Basis]
 * 출입국관리법 시행령 별표1 제28호의2 — F-6: 결혼이민자
 * 대한민국 국민의 배우자, 활동 제한 없음
 */
...
export class F6FulltimeEvaluator implements IFulltimeVisaEvaluator {
  readonly visaCode = 'F-6';
  readonly visaName = '결혼이민';
  readonly visaNameEn = 'Marriage Immigration';

  /**
   * Job-side evaluation: F-6는 모든 공고에 적합
   * Job-side evaluation: F-6 is eligible for all job postings
   */
  evaluateJob(_input: FulltimeJobInput): FulltimeVisaEvalResult {
    ...
    result.notes =
      'F-6 결혼이민비자 — 내국인과 동일한 취업 권리, 모든 직종 취업 가능, 별도 허가 불필요 ' +
      '(F-6 Marriage Immigration — Same employment rights as Korean nationals, ' +
  ...
  evaluateApplicant(
    ...
    // F-6는 job-side와 동일 — 지원자 조건과 무관하게 항상 적합
    // F-6 is same as job-side — always eligible regardless of applicant conditions
