...
export class JobPaymentCronService {
  ...
  async handleExpiredPremiums() {
    ...
    const executed = await this.lock.withLock(
      'cron:job-premium-expire',
      600, // 10분 TTL — 대량 처리 가능성 / 10-minute TTL for batch processing
      async () => {
        const now = new Date();
        ...
        const uniqueCorpIds = [...new Set(corporateIds)].map((id) =>
          BigInt(id),
        );
    ...
    if (!executed) {
      this.logger.debug(
        '[Cron:PremiumExpire] 다른 인스턴스에서 실행 중 — 스킵 / Another instance running — skipped',
      );
    }
  }
}
