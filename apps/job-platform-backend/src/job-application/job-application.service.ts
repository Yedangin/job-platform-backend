...
export class JobApplicationService {
  ...
  // ========================================
  // 12. 면접 제안 (Propose Interview - Employer/Applicant)
  // ========================================
  ...
  /**
   * 면접 확정 이메일 발송 / Send interview confirmation email
   * Triggered when applicant accepts an interview proposal (status -> CONFIRMED)
   */
  async sendInterviewConfirmationEmail(
    ...
    // proposedTime (set in acceptInterview) takes priority, then interviewDate
    const confirmedTime = app.proposedTime || app.interviewDate;

    // 면접 방식 결정 / Determine interview method
    // Use the application-level interviewMethod if set, otherwise fall back to job posting default
    ...
    if (corp) {
      ...
      // 기업 담당자에게 확정 이메일 발송 / Send confirmation email to employer
      if (corpEmail) {
      ...
      // 기업 담당자에게 인앱 알림 생성 / Create in-app notification for employer
      setImmediate(() => {
        this.createInAppNotification(
          corp.authId,
          'INTERVIEW_UPDATE',
   ...
   * ONLINE 면접일 경우 미팅 링크를 즉시 표시 / For ONLINE interviews, meeting link is displayed immediately
   */
  private getInterviewConfirmationEmailTemplate(params: {
    ...
    // 면접 방식별 상세 정보 / Method-specific details
    const methodLabel = isOnline
      ? '온라인 면접 / Online Interview'
      : '오프라인 면접 / Offline Interview';

    // ONLINE: 미팅 링크를 즉시 표시 / Show meeting link immediately for ONLINE interviews
           ...
           ${
             interviewLink
               ? `<p style="font-size: 14px; color: #333; margin: 0 0 8px 0;">
                  ...
                  <a href="${interviewLink}" style="display: inline-block; background-color: #28a745; color: #ffffff; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-size: 14px; font-weight: bold; margin-top: 4px;">
                    미팅 참여하기 / Join Meeting
                  </a>
                  <p style="font-size: 12px; color: #666; margin-top: 8px; word-break: break-all;">
                    ${interviewLink}
                  </p>`
               : `<p style="font-size: 13px; color: #666; margin: 0;">
                    미팅 링크는 별도 안내드립니다. / Meeting link will be provided separately.
                  </p>`
           }
         </div>`
           ...
           ${
             interviewLocation
               ? `<p style="font-size: 14px; color: #333; margin: 0;">
                    면접 장소 / Location: <strong>${interviewLocation}</strong>
                  </p>`
               : `<p style="font-size: 13px; color: #666; margin: 0;">
                    면접 장소는 별도 안내드립니다. / Interview location will be provided separately.
                  </p>`
           }
         </div>`;

    // 수신자별 본문 메시지 / Recipient-specific body message
    const bodyMessage = isApplicant
      ? `<p style="font-size: 16px; color: #28a745; font-weight: bold;">
           면접 일정이 확정되었습니다.<br>
           Your interview schedule has been confirmed.
         </p>
         <p style="font-size: 14px; color: #555; margin-top: 8px;">
           아래 일정에 맞춰 면접에 참석해주세요.<br>
           Please attend the interview at the scheduled time below.
         </p>`
      : `<p style="font-size: 16px; color: #28a745; font-weight: bold;">
           면접 일정이 확정되었습니다.<br>
           The interview schedule has been confirmed.
         </p>
         <p style="font-size: 14px; color: #555; margin-top: 8px;">
           지원자가 면접 일정을 수락하였습니다.<br>
           The applicant has accepted the interview schedule.
         </p>`;
        ...
        <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 40px; border-radius: 8px; margin-top: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
          ...
          <div style="border-top: 1px solid #eee; border-bottom: 1px solid #eee; padding: 30px 0;">
            ...
            <div style="margin: 20px 0; padding: 16px; background-color: #f0f7ff; border-radius: 8px; text-align: center;">
              ...
              <p style="font-size: 20px; color: #007bff; font-weight: bold; margin: 0;">
                ${confirmedTimeStr}
              </p>
            </div>
            ${methodDetailHtml}
