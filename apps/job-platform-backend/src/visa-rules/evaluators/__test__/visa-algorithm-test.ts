...
function mockVisaType(
  overrides: Partial<VisaTypeWithRelations> & { code: string; nameKo: string },
): VisaTypeWithRelations {
  return {
    id: BigInt(1),
    nameEn: null,
...
function baseInput(
  overrides: Partial<EvaluateVisaInput> = {},
): EvaluateVisaInput {
  return {
    ksicCode: '25',
    companySizeType: 'SME',
...
const H2_NAMES: Record<string, string> = {
  CN: '중국',
  RU: '러시아',
  UZ: '우즈베키스탄',
  KZ: '카자흐스탄',
...
const E2_NAMES: Record<string, string> = {
  US: '미국',
  GB: '영국',
  CA: '캐나다',
  AU: '호주',
...
const e9Type = mockVisaType({
  ...
  countryRestrictions: E9_MOU_COUNTRIES.map((c) => ({
    countryCode: c,
    countryNameKo: E9_MOU_NAMES[c],
    restrictionType: 'MOU_REQUIRED',
  })),
  requiredDocuments: [
    {
      documentName: '여권 사본',
      documentCode: 'PASSPORT_COPY',
      isRequired: true,
      conditionDesc: null,
    },
    {
      documentName: '표준근로계약서',
      documentCode: 'STANDARD_LABOR_CONTRACT',
      isRequired: true,
      conditionDesc: null,
    },
    {
      documentName: '건강진단서',
      documentCode: 'HEALTH_CERTIFICATE',
      isRequired: true,
      conditionDesc: null,
    },
    {
      documentName: 'EPS-TOPIK 합격증',
      documentCode: 'EPS_TOPIK',
      isRequired: true,
      conditionDesc: null,
    },
    {
      documentName: '고용허가서',
      documentCode: 'EMPLOYMENT_PERMIT',
      isRequired: true,
      conditionDesc: null,
    },
  ],
...
const e7Type = mockVisaType({
  ...
  occupationMappings: [
    {
      occupationCode: { kscoCode: '22', nameKo: '정보통신 전문가' },
      isAllowed: true,
    },
    {
      occupationCode: { kscoCode: '23', nameKo: '공학 전문가' },
      isAllowed: true,
    },
    {
      occupationCode: { kscoCode: '21', nameKo: '과학 전문가' },
      isAllowed: true,
    },
    {
      occupationCode: { kscoCode: '25', nameKo: '교육 전문가' },
      isAllowed: true,
    },
    {
      occupationCode: { kscoCode: '27', nameKo: '경영금융 전문가' },
      isAllowed: true,
    },
    {
      occupationCode: { kscoCode: '28', nameKo: '문화예술 전문가' },
      isAllowed: true,
    },
    {
      occupationCode: { kscoCode: '13', nameKo: '전문서비스 관리자' },
      isAllowed: true,
    },
  ...
  requiredDocuments: [
    ...
    {
      documentName: '고용계약서',
      documentCode: 'EMPLOYMENT_CONTRACT',
      isRequired: true,
      conditionDesc: null,
    },
    {
      documentName: '학위증명서',
      documentCode: 'DEGREE_CERT',
      isRequired: true,
      conditionDesc: null,
    },
  ],
...
const e2Type = mockVisaType({
  ...
  nameKo: '회화지도',
  employmentLevel: 'CONDITIONAL',
  countryRestrictions: E2_COUNTRIES.map((c) => ({
    countryCode: c,
    countryNameKo: E2_NAMES[c],
  ...
  requiredDocuments: [
    ...
    {
      documentName: '범죄경력 증명서',
      documentCode: 'CRIMINAL_RECORD_APOSTILLE',
      isRequired: true,
      conditionDesc: null,
    },
    {
...
const h1Type = mockVisaType({
  ...
  minAge: 18,
  maxAge: 30,
  countryRestrictions: H1_WH_COUNTRIES.map((c) => ({
    countryCode: c,
    countryNameKo: H1_WH_NAMES[c],
    restrictionType: 'ALLOWED',
  })),
  requiredDocuments: [
    {
    ...
    {
      documentName: '왕복항공권 예약확인서',
      documentCode: 'FLIGHT_BOOKING',
      isRequired: true,
      conditionDesc: null,
    },
    {
      documentName: '여행자보험',
      documentCode: 'TRAVEL_INSURANCE',
      isRequired: true,
      conditionDesc: null,
    },
  ],
...
const h2Type = mockVisaType({
  ...
  maxStayMonths: 58,
  renewalPossible: true,
  countryRestrictions: H2_COUNTRIES.map((c) => ({
    countryCode: c,
    countryNameKo: H2_NAMES[c],
    restrictionType: 'ALLOWED',
  })),
  requiredDocuments: [
    {
    ...
    {
      documentName: '재외동포 증명서류',
      documentCode: 'OVERSEAS_KOREAN_CERT',
      isRequired: true,
      conditionDesc: null,
    },
  ],
...
const f5Type = mockVisaType({
  code: 'F-5',
  nameKo: '영주',
  category: 'RESIDENCE',
  employmentLevel: 'FULL',
});
const f6Type = mockVisaType({
  code: 'F-6',
  nameKo: '결혼이민',
  category: 'RESIDENCE',
  employmentLevel: 'FULL',
});
const f1Type = mockVisaType({
  code: 'F-1',
  nameKo: '방문동거',
  category: 'RESIDENCE',
  employmentLevel: 'PROHIBITED',
});
const f3Type = mockVisaType({
  code: 'F-3',
  nameKo: '동반',
  category: 'RESIDENCE',
  employmentLevel: 'PROHIBITED',
});
const c4Type = mockVisaType({
  code: 'C-4',
  nameKo: '단기취업',
  employmentLevel: 'CONDITIONAL',
  maxStayMonths: 3,
...
function runTest(tc: TestCase, result: VisaEvaluation) {
  ...
  if (result.eligible !== tc.expected.eligible) {
    fails.push(
      `eligible: expected=${tc.expected.eligible}, actual=${result.eligible}`,
    );
  }
  if (
    tc.expected.blockedReasonContains &&
    !result.blockedReasons.some((r) =>
      r.includes(tc.expected.blockedReasonContains!),
    )
  ) {
    fails.push(
      `blockedReason should contain "${tc.expected.blockedReasonContains}", got: [${result.blockedReasons.join('; ')}]`,
    );
  }
  if (
    tc.expected.restrictionContains &&
    !result.restrictions.some((r) =>
      r.includes(tc.expected.restrictionContains!),
    )
  ) {
    fails.push(
      `restriction should contain "${tc.expected.restrictionContains}", got: [${result.restrictions.join('; ')}]`,
    );
  }
  if (
    tc.expected.noteContains &&
    !result.notes.some((n) => n.includes(tc.expected.noteContains!))
  ) {
    fails.push(
      `note should contain "${tc.expected.noteContains}", got: [${result.notes.join('; ')}]`,
    );
  }
  if (
    tc.expected.suggestionContains &&
    !result.suggestions.some((s) => s.includes(tc.expected.suggestionContains!))
  ) {
    fails.push(
      `suggestion should contain "${tc.expected.suggestionContains}", got: [${result.suggestions.join('; ')}]`,
    );
  }
  if (
    tc.expected.scoreGte !== undefined &&
    (result.score === undefined || result.score < tc.expected.scoreGte)
  ) {
    fails.push(
      `score should be >= ${tc.expected.scoreGte}, got: ${result.score}`,
    );
  }
  if (
    tc.expected.scoreLt !== undefined &&
    result.score !== undefined &&
    result.score >= tc.expected.scoreLt
  ) {
    fails.push(
      `score should be < ${tc.expected.scoreLt}, got: ${result.score}`,
    );
  }
  ...
  if (fails.length > 0) {
    failedTests++;
    failures.push({
      name: `[${tc.evaluator}] ${tc.name}`,
      expected: fails.join(' | '),
      actual: `eligible=${result.eligible}, blocked=[${result.blockedReasons.join(';')}], score=${result.score}`,
    });
  } else {
    passedTests++;
  }
...
for (const country of nonMouCountries) {
  const r = e9.evaluate(
    baseInput({ nationality: country, ksicCode: '25' }),
    e9Type,
  );
  runTest(
    {
...
for (const ksic of E9_ALLOWED) {
  const r = e9.evaluate(
    baseInput({ nationality: 'VN', ksicCode: ksic }),
    e9Type,
  );
  runTest(
    {
...
runTest(
  ...
  e9.evaluate(
    baseInput({ nationality: 'VN', companySizeType: 'LARGE' }),
    e9Type,
  ),
);
...
runTest(
  ...
  e7.evaluate(
    baseInput({ educationLevel: 'COLLEGE', offeredSalary: 300 }),
    e7Type,
  ),
);
...
runTest(
  ...
  e2.evaluate(
    baseInput({ nationality: 'US', educationLevel: 'COLLEGE', ksicCode: '85' }),
    e2Type,
  ),
);
...
runTest(
  ...
  h1.evaluate(
    baseInput({ nationality: 'AU', age: 25, ksicCode: '56221' }),
    h1Type,
  ),
);
...
for (const country of H2_COUNTRIES) {
  runTest(
    ...
    h2.evaluate(
      baseInput({ nationality: country, isEthnicKorean: true, ksicCode: '25' }),
      h2Type,
    ),
  );
}
...
for (const ksic of H2_ALLOWED_PREFIXES.slice(0, 15)) {
  runTest(
    ...
    h2.evaluate(
      baseInput({ isEthnicKorean: true, nationality: 'CN', ksicCode: ksic }),
      h2Type,
    ),
  );
}
  ...
  runTest(
    {
      ...
      visaType: f27Type,
      expected: {
        scoreGte: 25 + tc.score + 20 + 25,
        eligible: 25 + tc.score + 20 + 25 >= 80,
      }, // with age+korean+income
    },
    r,
  );
}
...
for (const occ of f4BlockedOccupations) {
  runTest(
    ...
    f4.evaluate(
      baseInput({ isEthnicKorean: true, targetOccupationCode: occ }),
      f4Type,
    ),
  );
}
    ...
    runTest(
      {
        ...
        expected: {
          eligible: false,
          blockedReasonContains: '취업 활동이 허용되지 않음',
        },
      },
...
runTest(
  ...
  eSimple.evaluate(
    baseInput({ educationLevel: 'MASTER', ksicCode: '85' }),
    eSimpleTypes['E-1'],
  ),
);
...
runTest(
  ...
  eSimple.evaluate(
    baseInput({ educationLevel: 'BACHELOR', ksicCode: '85' }),
    eSimpleTypes['E-1'],
  ),
);
...
runTest(
  ...
  eSimple.evaluate(
    baseInput({ educationLevel: 'MASTER', ksicCode: '70' }),
    eSimpleTypes['E-3'],
  ),
);
...
runTest(
  ...
  eSimple.evaluate(
    baseInput({ educationLevel: 'BACHELOR' }),
    eSimpleTypes['E-4'],
  ),
);
...
runTest(
  ...
  eSimple.evaluate(
    baseInput({ educationLevel: 'COLLEGE' }),
    eSimpleTypes['E-4'],
  ),
);
...
runTest(
  ...
  eSimple.evaluate(
    baseInput({ educationLevel: 'HIGH_SCHOOL', ksicCode: '90' }),
    eSimpleTypes['E-6'],
  ),
);
...
runTest(
  ...
  h2.evaluate(
    baseInput({ nationality: 'CN', isEthnicKorean: true, ksicCode: '41' }),
    h2Type,
  ),
);
...
runTest(
  ...
  h2.evaluate(
    baseInput({ isEthnicKorean: true, nationality: 'CN', ksicCode: '75' }),
    h2Type,
  ),
);
...
for (const ksic of e1AllowedKsic) {
  runTest(
    ...
    eSimple.evaluate(
      baseInput({ educationLevel: 'MASTER', ksicCode: ksic }),
      eSimpleTypes['E-1'],
    ),
  );
}
...
const e7EduExpCases = [
  ...
  },
  {
    ...
    salary: 300,
    eligible: false,
    desc: '고졸 경력3 LARGE blocked (5년필요)',
  },
  {
    edu: 'HIGH_SCHOOL',
    exp: 4,
  ...
  runTest(
    {
      ...
      input: baseInput({
        educationLevel: tc.edu,
        workExperienceYears: tc.exp,
        companySizeType: tc.sizeType,
        offeredSalary: tc.salary,
      }),
      visaType: e7Type,
      expected: { eligible: tc.eligible },
  ...
  runTest(
    {
      ...
      input: baseInput({
        nationality: 'VN',
        employeeCountKorean: tc.korean,
        employeeCountForeign: tc.foreign,
      }),
      visaType: e9Type,
      expected: { eligible: tc.eligible },
    },
...
for (const tc of e9SizeCases) {
  runTest(
    ...
    e9.evaluate(
      baseInput({ nationality: 'VN', companySizeType: tc.size }),
      e9Type,
    ),
  );
}
...
runTest(
  ...
  e9.evaluate(
    baseInput({ nationality: undefined, companySizeType: 'SME' }),
    e9Type,
  ),
);
...
runTest(
  {
    ...
    input: baseInput({
      educationLevel: 'BACHELOR',
      offeredSalary: 300,
      targetOccupationCode: undefined,
...
runTest(
  ...
  e2.evaluate(
    baseInput({ nationality: 'US', educationLevel: undefined, ksicCode: '85' }),
    e2Type,
  ),
);
...
runTest(
  ...
  h2.evaluate(
    baseInput({ isEthnicKorean: undefined, nationality: 'CN' }),
    h2Type,
  ),
);
...
runTest(
  ...
  d2.evaluate(
    baseInput({ jobType: 'PART_TIME', koreanLevel: undefined, ksicCode: '56' }),
    d2Type,
  ),
);
...
runTest(
  ...
  h2.evaluate(
    { ...itCompanyInput, isEthnicKorean: true, nationality: 'CN' },
    h2Type,
  ),
);
...
runTest(
  ...
  h2.evaluate(
    { ...constInput, isEthnicKorean: true, nationality: 'CN' },
    h2Type,
  ),
);
...
runTest(
  ...
  eSimple.evaluate(
    { ...constInput, educationLevel: 'MASTER' },
    eSimpleTypes['E-1'],
  ),
);
...
runTest(
  ...
  h2.evaluate(
    { ...farmInput, isEthnicKorean: true, nationality: 'CN' },
    h2Type,
  ),
);
...
runTest(
  {
    ...
    evaluator: 'E-9',
    input: { ...adultInput, nationality: 'VN' },
    visaType: e9Type,
    expected: { eligible: false, blockedReasonContains: '유흥업소' },
  },
...
runTest(
  {
    ...
    evaluator: 'H-1',
    input: { ...adultInput, nationality: 'AU', age: 25 },
    visaType: h1Type,
    expected: { eligible: false },
  },
...
runTest(
  {
    ...
    evaluator: 'D-2',
    input: { ...adultInput, jobType: 'PART_TIME' },
    visaType: d2Type,
    expected: { eligible: false },
  },
...
runTest(
  {
    ...
    evaluator: 'F-4',
    input: { ...adultInput, isEthnicKorean: true },
    visaType: f4Type,
    expected: { eligible: false },
  },
...
runTest(
  {
    ...
    evaluator: 'E-9',
    input: { ...maritimeInput, nationality: 'VN' },
    visaType: e9Type,
    expected: { eligible: false },
  },
...
runTest(
  ...
  e2.evaluate(
    baseInput({ nationality: 'US', educationLevel: 'MASTER', ksicCode: '85' }),
    e2Type,
  ),
);
...
runTest(
  ...
  e2.evaluate(
    baseInput({ nationality: 'US', educationLevel: 'DOCTOR', ksicCode: '85' }),
    e2Type,
  ),
);
...
runTest(
  ...
  e7.evaluate(
    baseInput({ offeredSalary: 10000, educationLevel: 'BACHELOR' }),
    e7Type,
  ),
);
...
runTest(
  ...
  e7.evaluate(
    baseInput({ offeredSalary: 0, educationLevel: 'BACHELOR' }),
    e7Type,
  ),
);
...
runTest(
  {
    ...
    input: baseInput({
      nationality: 'VN',
      employeeCountKorean: 100,
      employeeCountForeign: 1000,
    }),
    visaType: e9Type,
    expected: { eligible: false },
  },
...
for (const ksic of h1AllowedIndustries) {
  runTest(
    ...
    h1.evaluate(
      baseInput({ nationality: 'AU', age: 25, ksicCode: ksic }),
      h1Type,
    ),
  );
}
...
for (const code of h2SubCodes) {
  ...
  runTest(
    ...
    h2.evaluate(
      baseInput({ isEthnicKorean: true, nationality: 'CN', ksicCode: '25' }),
      vt,
    ),
  );
  ...
  runTest(
    ...
    h2.evaluate(
      baseInput({ isEthnicKorean: true, nationality: 'CN', ksicCode: '62' }),
      vt,
    ),
  );
}
  ...
  runTest(
    {
      ...
      evaluator: 'F-5',
      input: baseInput({ ksicCode: ind.ksic }),
      visaType: f5Type,
      expected: { eligible: true },
    },
  ...
  runTest(
    {
      ...
      evaluator: 'F-4',
      input: baseInput({ isEthnicKorean: true, targetOccupationCode: t.ksco }),
      visaType: f4Type,
      expected: { eligible: t.expect },
    },
    f4.evaluate(
      baseInput({ isEthnicKorean: true, targetOccupationCode: t.ksco }),
      f4Type,
    ),
  );
}
  ...
  runTest(
    {
      ...
      evaluator: 'H-1',
      input: baseInput({ nationality: 'AU', age: c.age }),
      visaType: h1Type,
...
runTest(
  {
    ...
    evaluator: 'H-1',
    input: baseInput({ nationality: 'AU', age: 25, ksicCode: '5611' }),
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(
    baseInput({ nationality: 'AU', age: 25, ksicCode: '5611' }),
    h1Type,
  ),
);
...
runTest(
  ...
  h2.evaluate(
    baseInput({ nationality: 'CN', isEthnicKorean: false, ksicCode: '25' }),
    h2Type,
  ),
);
...
runTest(
  {
    ...
    evaluator: 'H-2',
    input: baseInput({ nationality: 'CN', ksicCode: '25' }),
    visaType: h2Type,
    expected: { eligible: true },
  },
...
runTest(
  {
    ...
    evaluator: 'H-2',
    input: baseInput({ ksicCode: '25', koreanAncestryCountry: 'RU' }),
    visaType: h2Type,
    expected: { eligible: true },
  },
  h2.evaluate(
    baseInput({ ksicCode: '25', koreanAncestryCountry: 'RU' }),
    h2Type,
  ),
);
...
runTest(
  {
    ...
    evaluator: 'E-9',
    input: baseInput({ nationality: 'PH', ksicCode: '25' }),
    visaType: e9Type,
...
runTest(
  {
    ...
    evaluator: 'E-1',
    input: baseInput({ educationLevel: 'DOCTOR', ksicCode: '85' }),
    visaType: e1TypeEdu,
    expected: { eligible: true },
  },
  eSimple.evaluate(
    baseInput({ educationLevel: 'DOCTOR', ksicCode: '85' }),
    e1TypeEdu,
  ),
);
...
runTest(
  {
    ...
    evaluator: 'E-1',
    input: baseInput({ educationLevel: 'MASTER', ksicCode: '62' }),
    visaType: e1TypeEdu,
    expected: { eligible: false },
  },
  eSimple.evaluate(
    baseInput({ educationLevel: 'MASTER', ksicCode: '62' }),
    e1TypeEdu,
  ),
);
...
runTest(
  {
    ...
    evaluator: 'E-3',
    input: baseInput({ educationLevel: 'MASTER', ksicCode: '72' }),
    visaType: e3TypeRes,
    expected: { eligible: true },
  },
  eSimple.evaluate(
    baseInput({ educationLevel: 'MASTER', ksicCode: '72' }),
    e3TypeRes,
  ),
);
...
runTest(
  {
    ...
    evaluator: 'E-3',
    input: baseInput({ educationLevel: 'BACHELOR', ksicCode: '70' }),
    visaType: e3TypeRes,
    expected: { eligible: false },
  },
  eSimple.evaluate(
    baseInput({ educationLevel: 'BACHELOR', ksicCode: '70' }),
    e3TypeRes,
  ),
);
...
for (const tc of b2E9RatioCases) {
  const inp = baseInput({
    nationality: 'VN',
    employeeCountKorean: tc.k,
    employeeCountForeign: tc.f,
  });
  runTest(
    {
...
const b2E7SalaryLarge = [
  { salary: 200, eligible: false },
  { salary: 250, eligible: false },
  { salary: 270, eligible: false },
  { salary: 288, eligible: false },
  { salary: 289, eligible: false },
  { salary: 290, eligible: true },
  { salary: 295, eligible: true },
  { salary: 350, eligible: true },
  { salary: 1000, eligible: true },
  { salary: 5000, eligible: true },
];
...
const b2E7SalarySme = [
  { salary: 200, eligible: false },
  { salary: 230, eligible: false },
  { salary: 248, eligible: false },
  { salary: 249, eligible: false },
  { salary: 250, eligible: true },
  { salary: 255, eligible: true },
  { salary: 270, eligible: true },
  { salary: 290, eligible: true },
  { salary: 500, eligible: true },
  { salary: 2000, eligible: true },
];
...
const b2D2TopikCases = [
  { level: 'TOPIK1', hasNote: true },
  { level: 'TOPIK2', hasNote: true },
  { level: 'TOPIK3', hasNote: true },
  { level: 'TOPIK4', hasNote: false },
  { level: 'TOPIK5', hasNote: false },
  { level: 'TOPIK6', hasNote: false },
];
...
const b2F4OccTests = [
  { ksco: '1111', expect: true },
  { ksco: '1211', expect: true },
  { ksco: '2111', expect: true },
  { ksco: '2221', expect: true },
  { ksco: '3112', expect: true },
  { ksco: '4113', expect: true },
  { ksco: '5112', expect: true },
  { ksco: '6112', expect: true },
  { ksco: '7112', expect: true },
  { ksco: '8112', expect: true },
  { ksco: '9112', expect: false },
  { ksco: '9213', expect: false },
  { ksco: '9312', expect: false },
  { ksco: '9411', expect: false },
  { ksco: '9511', expect: false },
  { ksco: '9611', expect: false },
  { ksco: '9711', expect: false },
  { ksco: '9811', expect: false },
  { ksco: '9911', expect: false },
  { ksco: '9001', expect: false },
];
...
const b2E1EduKsic = [
  { edu: 'MASTER', ksic: '8511', e: true },
  { edu: 'MASTER', ksic: '8521', e: true },
  { edu: 'DOCTOR', ksic: '8530', e: true },
  { edu: 'DOCTOR', ksic: '8541', e: true },
  { edu: 'MASTER', ksic: '62', e: false },
  { edu: 'MASTER', ksic: '25', e: false },
  { edu: 'BACHELOR', ksic: '85', e: false },
  { edu: 'HIGH_SCHOOL', ksic: '85', e: false },
  { edu: 'COLLEGE', ksic: '85', e: false },
  { edu: 'DOCTOR', ksic: '70', e: false },
];
...
const b2E3Cases = [
  { edu: 'MASTER', ksic: '70', e: true },
  { edu: 'DOCTOR', ksic: '72', e: true },
  { edu: 'MASTER', ksic: '73', e: true },
  { edu: 'DOCTOR', ksic: '21', e: true },
  { edu: 'MASTER', ksic: '7211', e: true },
  { edu: 'DOCTOR', ksic: '7219', e: true },
  { edu: 'BACHELOR', ksic: '70', e: false },
  { edu: 'COLLEGE', ksic: '72', e: false },
  { edu: 'MASTER', ksic: '62', e: false },
  { edu: 'DOCTOR', ksic: '85', e: false },
];
...
runTest(
  {
    name: 'B2 프로필1 VN공장 → E-9 eligible',
    evaluator: 'E-9',
    input: vnWorkerInp,
    visaType: e9Type,
    expected: { eligible: true },
  },
  e9.evaluate(vnWorkerInp, e9Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → E-7 blocked(학력/급여)',
    evaluator: 'E-7',
    input: vnWorkerInp,
    visaType: e7Type,
    expected: { eligible: false },
  },
  e7.evaluate(vnWorkerInp, e7Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → H-1 blocked(비협정)',
    evaluator: 'H-1',
    input: vnWorkerInp,
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(vnWorkerInp, h1Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → H-2 blocked(비동포)',
    evaluator: 'H-2',
    input: vnWorkerInp,
    visaType: h2Type,
    expected: { eligible: false },
  },
  h2.evaluate(vnWorkerInp, h2Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → F-4 blocked(비동포)',
    evaluator: 'F-4',
    input: vnWorkerInp,
    visaType: f4Type,
    expected: { eligible: false },
  },
  f4.evaluate(vnWorkerInp, f4Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → F-5 eligible',
    evaluator: 'F-5',
    input: vnWorkerInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(vnWorkerInp, f5Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → C-4 eligible',
    evaluator: 'C-4',
    input: vnWorkerInp,
    visaType: c4Type,
    expected: { eligible: true },
  },
  c4.evaluate(vnWorkerInp, c4Type),
);
runTest(
  {
    name: 'B2 프로필1 VN공장 → B-2 blocked',
    evaluator: 'B-2',
    input: vnWorkerInp,
    visaType: prohibitedTypes['B-2'],
    expected: { eligible: false },
  },
  prohibited.evaluate(vnWorkerInp, prohibitedTypes['B-2']),
);
...
runTest(
  {
    name: 'B2 프로필2 US IT개발 → E-9 blocked(비MOU)',
    evaluator: 'E-9',
    input: usDevInp,
    visaType: e9Type,
    expected: { eligible: false },
  },
  e9.evaluate(usDevInp, e9Type),
);
runTest(
  {
    name: 'B2 프로필2 US IT개발 → E-7 eligible',
    evaluator: 'E-7',
    input: usDevInp,
    visaType: e7Type,
    expected: { eligible: true },
  },
  e7.evaluate(usDevInp, e7Type),
);
runTest(
  {
    name: 'B2 프로필2 US IT개발 → E-2 eligible(+restrict)',
    evaluator: 'E-2',
    input: usDevInp,
    visaType: e2Type,
    expected: { eligible: true },
  },
  e2.evaluate(usDevInp, e2Type),
);
runTest(
  {
    name: 'B2 프로필2 US IT개발 → H-1 blocked(나이X)',
    evaluator: 'H-1',
    input: usDevInp,
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(usDevInp, h1Type),
);
runTest(
  {
    name: 'B2 프로필2 US IT개발 → F-5 eligible',
    evaluator: 'F-5',
    input: usDevInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(usDevInp, f5Type),
);
runTest(
  {
    name: 'B2 프로필2 US IT개발 → C-4 eligible',
    evaluator: 'C-4',
    input: usDevInp,
    visaType: c4Type,
    expected: { eligible: true },
  },
  c4.evaluate(usDevInp, c4Type),
);
...
runTest(
  {
    name: 'B2 프로필3 AU카페 → H-1 eligible',
    evaluator: 'H-1',
    input: auCafeInp,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(auCafeInp, h1Type),
);
runTest(
  {
    name: 'B2 프로필3 AU카페 → D-2 eligible(PT)',
    evaluator: 'D-2',
    input: auCafeInp,
    visaType: d2Type,
    expected: { eligible: true },
  },
  d2.evaluate(auCafeInp, d2Type),
);
runTest(
  {
    name: 'B2 프로필3 AU카페 → E-2 blocked(학력)',
    evaluator: 'E-2',
    input: auCafeInp,
    visaType: e2Type,
    expected: { eligible: false },
  },
  e2.evaluate(auCafeInp, e2Type),
);
runTest(
  {
    name: 'B2 프로필3 AU카페 → F-5 eligible',
    evaluator: 'F-5',
    input: auCafeInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(auCafeInp, f5Type),
);
...
const cnKoreanConstInp = baseInput({
  ...
  companySizeType: 'SME',
  educationLevel: 'HIGH_SCHOOL',
  employeeCountKorean: 20,
  employeeCountForeign: 3,
});
runTest(
  {
    name: 'B2 프로필4 CN동포 건설 → E-9 eligible',
    evaluator: 'E-9',
    input: cnKoreanConstInp,
    visaType: e9Type,
    expected: { eligible: true },
  },
  e9.evaluate(cnKoreanConstInp, e9Type),
);
runTest(
  {
    name: 'B2 프로필4 CN동포 건설 → H-2 eligible',
    evaluator: 'H-2',
    input: cnKoreanConstInp,
    visaType: h2Type,
    expected: { eligible: true },
  },
  h2.evaluate(cnKoreanConstInp, h2Type),
);
runTest(
  {
    name: 'B2 프로필4 CN동포 건설 → F-4 eligible',
    evaluator: 'F-4',
    input: cnKoreanConstInp,
    visaType: f4Type,
    expected: { eligible: true },
  },
  f4.evaluate(cnKoreanConstInp, f4Type),
);
runTest(
  {
    name: 'B2 프로필4 CN동포 건설 → F-5 eligible',
    evaluator: 'F-5',
    input: cnKoreanConstInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(cnKoreanConstInp, f5Type),
);
...
runTest(
  {
    name: 'B2 프로필5 JP연구 → E-7 eligible',
    evaluator: 'E-7',
    input: jpResearcherInp,
    visaType: e7Type,
    expected: { eligible: true },
  },
  e7.evaluate(jpResearcherInp, e7Type),
);
runTest(
  {
    name: 'B2 프로필5 JP연구 → E-3 eligible',
    evaluator: 'E-3',
    input: jpResearcherInp,
    visaType: eSimpleTypes['E-3'],
    expected: { eligible: true },
  },
  eSimple.evaluate(jpResearcherInp, eSimpleTypes['E-3']),
);
runTest(
  {
    name: 'B2 프로필5 JP연구 → H-1 eligible',
    evaluator: 'H-1',
    input: jpResearcherInp,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(jpResearcherInp, h1Type),
);
runTest(
  {
    name: 'B2 프로필5 JP연구 → E-9 blocked(비MOU)',
    evaluator: 'E-9',
    input: jpResearcherInp,
    visaType: e9Type,
    expected: { eligible: false },
  },
  e9.evaluate(jpResearcherInp, e9Type),
);
...
runTest(
  {
    name: 'B2 프로필6 GB교사 → E-2 eligible',
    evaluator: 'E-2',
    input: gbTeacherInp,
    visaType: e2Type,
    expected: { eligible: true },
  },
  e2.evaluate(gbTeacherInp, e2Type),
);
runTest(
  {
    name: 'B2 프로필6 GB교사 → H-1 eligible',
    evaluator: 'H-1',
    input: gbTeacherInp,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(gbTeacherInp, h1Type),
);
runTest(
  {
    name: 'B2 프로필6 GB교사 → E-1 blocked(석사필요)',
    evaluator: 'E-1',
    input: gbTeacherInp,
    visaType: eSimpleTypes['E-1'],
    expected: { eligible: false },
  },
  eSimple.evaluate(gbTeacherInp, eSimpleTypes['E-1']),
);
...
const ruKoreanRetailInp = baseInput({
  ...
  ksicCode: '47',
  companySizeType: 'SME',
  employeeCountKorean: 15,
  employeeCountForeign: 2,
});
runTest(
  {
    name: 'B2 프로필7 RU동포 유통 → H-2 eligible',
    evaluator: 'H-2',
    input: ruKoreanRetailInp,
    visaType: h2Type,
    expected: { eligible: true },
  },
  h2.evaluate(ruKoreanRetailInp, h2Type),
);
runTest(
  {
    name: 'B2 프로필7 RU동포 유통 → F-4 eligible',
    evaluator: 'F-4',
    input: ruKoreanRetailInp,
    visaType: f4Type,
    expected: { eligible: true },
  },
  f4.evaluate(ruKoreanRetailInp, f4Type),
);
runTest(
  {
    name: 'B2 프로필7 RU동포 유통 → E-9 blocked(비MOU)',
    evaluator: 'E-9',
    input: ruKoreanRetailInp,
    visaType: e9Type,
    expected: { eligible: false },
  },
  e9.evaluate(ruKoreanRetailInp, e9Type),
);
...
runTest(
  {
    name: 'B2 프로필8 PH농업 → E-9 eligible',
    evaluator: 'E-9',
    input: phFarmInp,
    visaType: e9Type,
    expected: { eligible: true },
  },
  e9.evaluate(phFarmInp, e9Type),
);
runTest(
  {
    name: 'B2 프로필8 PH농업 → E-8 eligible',
    evaluator: 'E-8',
    input: phFarmInp,
    visaType: eSimpleTypes['E-8'],
    expected: { eligible: true },
  },
  eSimple.evaluate(phFarmInp, eSimpleTypes['E-8']),
);
runTest(
  {
    name: 'B2 프로필8 PH농업 → E-10 blocked(비수상)',
    evaluator: 'E-10',
    input: phFarmInp,
    visaType: eSimpleTypes['E-10'],
    expected: { eligible: false },
  },
  eSimple.evaluate(phFarmInp, eSimpleTypes['E-10']),
);
...
runTest(
  {
    name: 'B2 프로필9 IN박사IT → E-7 eligible',
    evaluator: 'E-7',
    input: inDoctorInp,
    visaType: e7Type,
    expected: { eligible: true },
  },
  e7.evaluate(inDoctorInp, e7Type),
);
runTest(
  {
    name: 'B2 프로필9 IN박사IT → E-9 blocked(비MOU)',
    evaluator: 'E-9',
    input: inDoctorInp,
    visaType: e9Type,
    expected: { eligible: false },
  },
  e9.evaluate(inDoctorInp, e9Type),
);
runTest(
  {
    name: 'B2 프로필9 IN박사IT → H-1 blocked(비협정)',
    evaluator: 'H-1',
    input: inDoctorInp,
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(inDoctorInp, h1Type),
);
runTest(
  {
    name: 'B2 프로필9 IN박사IT → F-5 eligible',
    evaluator: 'F-5',
    input: inDoctorInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(inDoctorInp, f5Type),
);
...
runTest(
  {
    name: 'B2 프로필10 KZ동포 청소 → H-2 eligible(75허용)',
    evaluator: 'H-2',
    input: kzCleanerInp,
    visaType: h2Type,
    expected: { eligible: true },
  },
  h2.evaluate(kzCleanerInp, h2Type),
);
runTest(
  {
    name: 'B2 프로필10 KZ동포 청소 → F-4 blocked(청소업)',
    evaluator: 'F-4',
    input: kzCleanerInp,
    visaType: f4Type,
    expected: { eligible: false },
  },
  f4.evaluate(kzCleanerInp, f4Type),
);
...
runTest(
  {
    name: 'B2 프로필11 DE WH 숙박 → H-1 eligible',
    evaluator: 'H-1',
    input: deWhInp1,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(deWhInp1, h1Type),
);
runTest(
  {
    name: 'B2 프로필11 DE WH 음식 → H-1 eligible',
    evaluator: 'H-1',
    input: deWhInp2,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(deWhInp2, h1Type),
);
runTest(
  {
    name: 'B2 프로필11 DE WH 소매 → H-1 eligible',
    evaluator: 'H-1',
    input: deWhInp3,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(deWhInp3, h1Type),
);
runTest(
  {
    name: 'B2 프로필11 DE WH IT → H-1 eligible',
    evaluator: 'H-1',
    input: deWhInp4,
    visaType: h1Type,
    expected: { eligible: true },
  },
  h1.evaluate(deWhInp4, h1Type),
);

// 프로필 12: D-2 유학생 다양한 알바
const d2StudentInp1 = baseInput({
  jobType: 'PART_TIME',
  ksicCode: '47',
  koreanLevel: 'TOPIK3',
});
const d2StudentInp2 = baseInput({
  jobType: 'PART_TIME',
  ksicCode: '55',
  koreanLevel: 'TOPIK5',
});
const d2StudentInp3 = baseInput({
  jobType: 'PART_TIME',
  ksicCode: '62',
  koreanLevel: 'TOPIK4',
});
...
runTest(
  {
    name: 'B2 프로필12 D-2 소매PT → eligible',
    evaluator: 'D-2',
    input: d2StudentInp1,
    visaType: d2Type,
    expected: { eligible: true, noteContains: 'TOPIK 4급' },
  },
  d2.evaluate(d2StudentInp1, d2Type),
);
runTest(
  {
    name: 'B2 프로필12 D-2 숙박PT → eligible',
    evaluator: 'D-2',
    input: d2StudentInp2,
    visaType: d2Type,
    expected: { eligible: true },
  },
  d2.evaluate(d2StudentInp2, d2Type),
);
runTest(
  {
    name: 'B2 프로필12 D-2 IT PT → eligible',
    evaluator: 'D-2',
    input: d2StudentInp3,
    visaType: d2Type,
    expected: { eligible: true },
  },
  d2.evaluate(d2StudentInp3, d2Type),
);
runTest(
  {
    name: 'B2 프로필12 D-2 FT → blocked',
    evaluator: 'D-2',
    input: d2StudentInp4,
    visaType: d2Type,
    expected: { eligible: false },
  },
  d2.evaluate(d2StudentInp4, d2Type),
);
...
runTest(
  {
    name: 'B2 F-2-7 90점+범죄-5 → eligible',
    evaluator: 'F-2-7',
    input: b2F27Crim1,
    visaType: f27Type,
    expected: { eligible: true, noteContains: '범죄경력' },
  },
  f2.evaluate(b2F27Crim1, f27Type),
);

// 출입국위반만
const b2F27Imm1 = baseInput({
  age: 25,
  educationLevel: 'MASTER',
  koreanLevel: 'TOPIK6',
  incomeGniPercent: 200,
...
runTest(
  {
    name: 'B2 F-2-7 90점+위반-10 → eligible',
    evaluator: 'F-2-7',
    input: b2F27Imm1,
    visaType: f27Type,
    expected: { eligible: true, noteContains: '출입국법' },
  },
  f2.evaluate(b2F27Imm1, f27Type),
);

// 양쪽 다
const b2F27Both = baseInput({
  age: 25,
  educationLevel: 'MASTER',
  koreanLevel: 'TOPIK6',
  incomeGniPercent: 200,
...
runTest(
  {
    name: 'B2 F-2-7 90점+범죄+위반 → eligible',
    evaluator: 'F-2-7',
    input: b2F27Both,
    visaType: f27Type,
    expected: { eligible: true },
  },
  f2.evaluate(b2F27Both, f27Type),
);
...
for (const tc of b2F27PenaltyCases) {
  const inp = baseInput({
    ...
    koreanLevel: tc.kor,
    incomeGniPercent: tc.gni,
    hasCriminalRecord: tc.crim,
    hasImmigrationViolation: tc.imm,
  });
  const base =
    (tc.age <= 24
      ? 25
    ...
    (tc.edu === 'DOCTOR'
      ...
      : tc.edu === 'MASTER'
        ...
        : tc.edu === 'BACHELOR'
          ...
          : 15) +
    (tc.kor === 'TOPIK6' ? 20 : 16) +
    (tc.gni >= 300 ? 25 : tc.gni >= 200 ? 20 : 15);
...
runTest(
  {
    name: 'B2 E-9 nationality undefined eligible',
    evaluator: 'E-9',
    input: baseInput({ nationality: undefined, ksicCode: '25' }),
    visaType: e9Type,
    expected: { eligible: true },
  },
  e9.evaluate(baseInput({ nationality: undefined, ksicCode: '25' }), e9Type),
);

// E-7 targetOccupationCode 미입력 (safe)
runTest(
  {
    name: 'B2 E-7 occ undefined eligible',
    evaluator: 'E-7',
    input: baseInput({
      educationLevel: 'BACHELOR',
      offeredSalary: 300,
...
runTest(
  {
    name: 'B2 E-7 nationality undefined eligible',
    evaluator: 'E-7',
    input: baseInput({
      educationLevel: 'BACHELOR',
      offeredSalary: 300,
...
runTest(
  {
    name: 'B2 H-1 nationality undefined eligible',
    evaluator: 'H-1',
    input: baseInput({ nationality: undefined, age: 25 }),
    visaType: h1Type,
    expected: { eligible: true },
...
runTest(
  {
    name: 'B2 H-1 age undefined eligible',
    evaluator: 'H-1',
    input: baseInput({ nationality: 'AU', age: undefined }),
    visaType: h1Type,
    expected: { eligible: true },
...
runTest(
  {
    name: 'B2 H-2 isEthnicKorean undefined eligible',
    evaluator: 'H-2',
    input: baseInput({
      isEthnicKorean: undefined,
      nationality: 'CN',
      ksicCode: '25',
    }),
    visaType: h2Type,
  ...
  h2.evaluate(
    baseInput({ isEthnicKorean: undefined, nationality: 'CN', ksicCode: '25' }),
    h2Type,
  ),
);
...
runTest(
  {
    name: 'B2 F-4 isEthnicKorean undefined eligible',
    evaluator: 'F-4',
    input: baseInput({ isEthnicKorean: undefined }),
    visaType: f4Type,
    expected: { eligible: true },
...
runTest(
  {
    name: 'B2 F-2-7 koreanLevel undefined',
    evaluator: 'F-2-7',
    input: baseInput({
      age: 20,
      educationLevel: 'DOCTOR',
...
runTest(
  {
    name: 'B2 F-2-7 income undefined',
    evaluator: 'F-2-7',
    input: baseInput({
      age: 20,
      educationLevel: 'DOCTOR',
...
runTest(
  {
    name: 'B2 E-7 급여 1원 blocked',
    evaluator: 'E-7',
    input: baseInput({ offeredSalary: 1, educationLevel: 'BACHELOR' }),
    visaType: e7Type,
    expected: { eligible: false },
  },
  e7.evaluate(
    baseInput({ offeredSalary: 1, educationLevel: 'BACHELOR' }),
    e7Type,
  ),
);
runTest(
  {
    name: 'B2 E-7 급여 99999원 eligible',
    evaluator: 'E-7',
    input: baseInput({ offeredSalary: 99999, educationLevel: 'BACHELOR' }),
    visaType: e7Type,
    expected: { eligible: true },
  },
  e7.evaluate(
    baseInput({ offeredSalary: 99999, educationLevel: 'BACHELOR' }),
    e7Type,
  ),
);
runTest(
  {
    name: 'B2 E-9 외국인 0:0 eligible',
    evaluator: 'E-9',
    input: baseInput({
      nationality: 'VN',
      employeeCountKorean: 0,
...
runTest(
  {
    name: 'B2 H-1 나이 0세 blocked',
    evaluator: 'H-1',
    input: baseInput({ nationality: 'AU', age: 0 }),
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(baseInput({ nationality: 'AU', age: 0 }), h1Type),
);
runTest(
  {
    name: 'B2 H-1 나이 1세 blocked',
    evaluator: 'H-1',
    input: baseInput({ nationality: 'AU', age: 1 }),
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(baseInput({ nationality: 'AU', age: 1 }), h1Type),
);
runTest(
  {
    name: 'B2 H-1 나이 100세 blocked',
    evaluator: 'H-1',
    input: baseInput({ nationality: 'AU', age: 100 }),
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(baseInput({ nationality: 'AU', age: 100 }), h1Type),
);
runTest(
  {
    name: 'B2 F-2-7 나이 1세 점수',
    evaluator: 'F-2-7',
    input: baseInput({
      age: 1,
      educationLevel: 'DOCTOR',
...
runTest(
  {
    name: 'B2 F-2-7 GNI 1000% 점수',
    evaluator: 'F-2-7',
    input: baseInput({
      age: 20,
      educationLevel: 'DOCTOR',
      koreanLevel: 'TOPIK6',
      incomeGniPercent: 1000,
    }),
    visaType: f27Type,
    expected: { eligible: true, scoreGte: 105 },
  },
...
runTest(
  {
    name: 'B3 숙박 → E-9 eligible',
    evaluator: 'E-9',
    input: b3HotelInp,
    visaType: e9Type,
    expected: { eligible: true },
  },
  e9.evaluate(b3HotelInp, e9Type),
);
runTest(
  {
    name: 'B3 숙박 → E-7 blocked(학력/급여)',
    evaluator: 'E-7',
    input: b3HotelInp,
    visaType: e7Type,
    expected: { eligible: false },
  },
  e7.evaluate(b3HotelInp, e7Type),
);
runTest(
  {
    name: 'B3 숙박 → H-1 blocked(비협정)',
    evaluator: 'H-1',
    input: b3HotelInp,
    visaType: h1Type,
    expected: { eligible: false },
  },
  h1.evaluate(b3HotelInp, h1Type),
);
runTest(
  {
    name: 'B3 숙박 → F-5 eligible',
    evaluator: 'F-5',
    input: b3HotelInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(b3HotelInp, f5Type),
);
runTest(
  {
    name: 'B3 숙박 → F-6 eligible',
    evaluator: 'F-6',
    input: b3HotelInp,
    visaType: f6Type,
    expected: { eligible: true },
  },
  fSimple.evaluate(b3HotelInp, f6Type),
);
runTest(
  {
    name: 'B3 숙박 → C-4 eligible',
    evaluator: 'C-4',
    input: b3HotelInp,
    visaType: c4Type,
    expected: { eligible: true },
  },
  c4.evaluate(b3HotelInp, c4Type),
);
runTest(
  {
    name: 'B3 숙박 → D-2 PT eligible',
    evaluator: 'D-2',
    input: { ...b3HotelInp, jobType: 'PART_TIME' },
    visaType: d2Type,
    expected: { eligible: true },
  },
  d2.evaluate({ ...b3HotelInp, jobType: 'PART_TIME' }, d2Type),
);
runTest(
  {
    name: 'B3 숙박 → D-10 eligible',
    evaluator: 'D-10',
    input: b3HotelInp,
    visaType: dSimpleTypes['D-10'],
    expected: { eligible: true },
  },
  dSimple.evaluate(b3HotelInp, dSimpleTypes['D-10']),
);
runTest(
  {
    name: 'B3 숙박 → A-1 blocked',
    evaluator: 'A-1',
    input: b3HotelInp,
    visaType: prohibitedTypes['A-1'],
    expected: { eligible: false },
  },
  prohibited.evaluate(b3HotelInp, prohibitedTypes['A-1']),
);
runTest(
  {
    name: 'B3 숙박 → F-1 blocked',
    evaluator: 'F-1',
    input: b3HotelInp,
    visaType: f1Type,
    expected: { eligible: false },
  },
  fSimple.evaluate(b3HotelInp, f1Type),
);
...
runTest(
  {
    name: 'B3 음식 → E-9 eligible',
    evaluator: 'E-9',
    input: b3FoodInp,
    visaType: e9Type,
    expected: { eligible: true },
  },
  e9.evaluate(b3FoodInp, e9Type),
);
runTest(
  {
    name: 'B3 음식 → H-2 eligible',
    evaluator: 'H-2',
    input: { ...b3FoodInp, isEthnicKorean: true, nationality: 'CN' },
    visaType: h2Type,
    expected: { eligible: true },
  },
  h2.evaluate(
    { ...b3FoodInp, isEthnicKorean: true, nationality: 'CN' },
    h2Type,
  ),
);
runTest(
  {
    name: 'B3 음식 → F-4 eligible',
    evaluator: 'F-4',
    input: { ...b3FoodInp, isEthnicKorean: true },
    visaType: f4Type,
    expected: { eligible: true },
  },
  f4.evaluate({ ...b3FoodInp, isEthnicKorean: true }, f4Type),
);
runTest(
  {
    name: 'B3 음식 → F-5 eligible',
    evaluator: 'F-5',
    input: b3FoodInp,
    visaType: f5Type,
    expected: { eligible: true },
  },
  f5.evaluate(b3FoodInp, f5Type),
);
runTest(
  {
    name: 'B3 음식 → C-4 eligible',
    evaluator: 'C-4',
    input: b3FoodInp,
    visaType: c4Type,
    expected: { eligible: true },
  },
  c4.evaluate(b3FoodInp, c4Type),
);
runTest(
  {
    name: 'B3 음식 → E-6 blocked',
    evaluator: 'E-6',
    input: b3FoodInp,
    visaType: eSimpleTypes['E-6'],
    expected: { eligible: false },
  },
  eSimple.evaluate(b3FoodInp, eSimpleTypes['E-6']),
);
  ...
  runTest(
    {
      ...
      evaluator: 'D-2',
      input: baseInput({ jobType: 'FULL_TIME', ksicCode: ksic }),
      visaType: d2Type,
      expected: { eligible: false, blockedReasonContains: '풀타임' },
    },
...
for (const [code, vt] of Object.entries(prohibitedTypes)) {
  for (const cond of b3ProhConditions) {
    const inp = baseInput({
      educationLevel: cond.edu,
      offeredSalary: cond.sal,
    });
    runTest(
      {
...
const b3C4Conds = [
  { ksic: '56', job: 'FULL_TIME' },
  { ksic: '56', job: 'PART_TIME' },
  { ksic: '62', job: 'FULL_TIME' },
  { ksic: '85', job: 'FULL_TIME' },
  { ksic: '90', job: 'PART_TIME' },
  { ksic: '25', job: 'FULL_TIME' },
  { ksic: '41', job: 'PART_TIME' },
  { ksic: '01', job: 'FULL_TIME' },
  { ksic: '50', job: 'FULL_TIME' },
  { ksic: '70', job: 'PART_TIME' },
  { ksic: '96', job: 'FULL_TIME' },
  { ksic: '47', job: 'PART_TIME' },
  { ksic: '49', job: 'FULL_TIME' },
  { ksic: '64', job: 'PART_TIME' },
...
const b3F5Conds = [
  { ksic: '56221', job: 'FULL_TIME' },
  { ksic: '75', job: 'FULL_TIME' },
  { ksic: '62', job: 'PART_TIME' },
  { ksic: '85', job: 'PART_TIME' },
  { ksic: '90', job: 'FULL_TIME' },
  { ksic: '25', job: 'FULL_TIME' },
  { ksic: '41', job: 'PART_TIME' },
  { ksic: '01', job: 'FULL_TIME' },
  { ksic: '50', job: 'FULL_TIME' },
...
for (const tc of b3E7Combos) {
  const inp = baseInput({
    targetOccupationCode: tc.occ,
    offeredSalary: tc.sal,
    companySizeType: tc.size,
    educationLevel: tc.edu,
  });
  runTest(
...
for (const ksic of b3D4Industries) {
  runTest(
    ...
    dSimple.evaluate(
      baseInput({ jobType: 'PART_TIME', ksicCode: ksic }),
      dSimpleTypes['D-4'],
    ),
  );
  runTest(
    ...
    dSimple.evaluate(
      baseInput({ jobType: 'FULL_TIME', ksicCode: ksic }),
      dSimpleTypes['D-4'],
    ),
  );
}
