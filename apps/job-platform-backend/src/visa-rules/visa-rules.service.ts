  ...
  async getEvaluationLogs(page = 1, limit = 20) {
    ...
    return {
      data: logs.map((l) => ({
        ...
        policyReleaseId: l.policyReleaseId,
        policyVersion: l.policyVersion,
        policyHash: l.policyHash,
        policyAsOf: l.policyAsOf?.toISOString() ?? null,
        policyEffectiveFrom: l.policyEffectiveFrom?.toISOString() ?? null,
        policyReviewedAt: l.policyReviewedAt?.toISOString() ?? null,
        releaseId: l.policyReleaseId,
        releaseVersion: l.policyVersion,
        reviewStatus: l.policyReviewedAt ? 'APPROVED' : 'UNKNOWN',
      })),
      meta: { total, page, limit, totalPages: Math.ceil(total / limit) },
    };
  }
